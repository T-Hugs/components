...
const portalRegister: {[key: string]: Element} = {}
 ...
 */
export function registerPortal(root: Element, name?: string): void {
  portalRegister[name ?? DEFAULT_PORTAL_CONTAINER_NAME] = root
}
...
function ensureDefaultPortal() {
  if (!(DEFAULT_PORTAL_CONTAINER_NAME in portalRegister)) {
    ...
    portalRegister[DEFAULT_PORTAL_CONTAINER_NAME] = defaultPortalContainer
  }
}
...
export const Portal: React.FC<PortalProps> = ({children, onMount, containerName: _containerName}) => {
  ...
  const parentElement = portalRegister[containerName]
